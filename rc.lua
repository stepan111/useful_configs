...
-- {{{ Tags
tags = {
   names = {  " TERMINAL "," WEB ", " SKYPE ", " MAIL ", " OTHER " },
   layout = { layouts[1], layouts[2], layouts[3], layouts[4], layouts[5] }
}
    
for s = 1, screen.count() do
   if ( s == 2 ) then
	tags[s]=awful.tag({"TERMINAL", "OTHER"}, s, {layouts[1], layouts[2]} )
   else
...
globalkeys = awful.util.table.join(
    -- Show conky
    awful.key({}, "F9", function() raise_conky() end, function() lower_conky() end),
...
-- os.execute("xrandr --output DP2 --off")
local remove_DP2 = true
local xrandr = io.popen("xrandr -q")
   if xrandr then
	 ...
	 if (output == "DP2") then
		remove_DP2 = false
		os.execute("xrandr --output DP2 --auto")
		os.execute("xrandr --output DP2 --right-of eDP1")
		awful.rules.rules = {
  		  { rule = { },
		      properties = { border_width = beautiful.border_width,
                	     border_color = beautiful.border_normal,
